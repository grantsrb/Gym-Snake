...
class GridTests(unittest.TestCase):
    ...
    def grid_Initialization_Test(self):
        grid = Grid(self.grid_size, self.space_size)
        expected_grid = np.array((self.grid_size[1]*self.space_size,self.grid_size[0]*self.space_size, 3), dtype=np.uint8)
    ...
    def space_size_Initialization_Test(self):
        grid = Grid(self.grid_size, self.space_size)
        self.assertTrue(grid.space_size == self.space_size)
    ...
    def color_of_Coordinate_Test(self):
        ...
        grid.grid[coord[1]*self.space_size,coord[0]*self.space_size,:] = expected_color
    ...
    def draw_Positive_Test(self):
        ...
        for i in range(grid.grid.shape[0]):
            for j in range(grid.grid.shape[1]):
                if i >= coord[1] and i < coord[1]+grid.space_size and j >= coord[0] and j < coord[0]+grid.space_size:
                    self.assertTrue(np.array_equal(grid.grid[i,j,:],expected_color))
                else:
